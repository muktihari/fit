...
func (m *Record) Reset(mesg *proto.Message) {
	...
	if mesg != nil {
		knownNums := [4]uint64{5764606990190788607, 18013290270358914040, 2050, 2305843009213693952}
		...
		for i := range mesg.Fields {
			...
			if mesg.Fields[i].IsExpandedField && num < 109 {
				state[num>>3] |= 1 << (num & 7)
			}
